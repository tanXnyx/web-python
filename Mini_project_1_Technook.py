...
class Apply(tk.Frame): 
    ...
    def interface(self):
        ...
        self.power_button = tk.Button(self, text="Start", command=lambda: self.begin())
        self.power_button.grid(row=3, column=0, sticky="NE")

        self.reset_button = tk.Button(self, text="Reset", command=lambda: self.readjust())
        ...
        self.master.bind("<Return>", lambda x: self.begin())
        self.time_entry.bind("<Key>", lambda v: self.upgrade())
    ...
    def upgrade(self):
        ...
        try:
            self.clock.configure(text=self.cal())
        except:
    ...
    def timekeeper(self):
        ...
        if self.running:
            if self.time <= 0:
                self.clock.configure(text="end!")
            else:
                self.clock.configure(text=self.cal())
                self.time -= 1
                self.after(1000, self.timekeeper)
    ...
    def begin(self):
        ...
        try:
            ...
            self.time = self.time
        self.power_button.configure(text="Stop", command=lambda: self.finish())
    ...
    def finish(self):
        """Stop timer"""
        self.power_button.configure(text="Start", command=lambda: self.begin())
        self.master.bind("<Return>", lambda x: self.begin())
        self.running = False
...
if __name__ == "__main__":
    ...
    root = tk.Tk()
    root.title("TIMER")
    Apply(root).pack(side="top", fill="both", expand=True)
    root.mainloop()
